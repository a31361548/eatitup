...
export default async function TodosPage(): Promise<React.ReactElement> {
  ...
  const stats = [
    { label: 'Active', value: active, accent: 'text-cyan-200', border: 'border-cyan-500/50' },
    { label: 'Upcoming', value: upcoming, accent: 'text-amber-200', border: 'border-amber-400/40' },
    { label: 'Completed', value: completed, accent: 'text-emerald-200', border: 'border-emerald-400/40' },
  ]
  return (
    <HoloWindow title="RITUAL TASK FORGE" className="h-full">
      <div className="space-y-10">
        <section className="rounded-[40px] border border-white/10 bg-gradient-to-br from-white/10 via-transparent to-black/30 p-6 text-white shadow-[0_30px_90px_rgba(0,0,0,0.45)]">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              <p className="text-xs font-tech uppercase tracking-[0.45em] text-white/60">Aether Mission Suite</p>
            ...
            <div className="text-xs font-tech uppercase tracking-[0.4em] text-white/60">
              SYSTEM PROTOCOL: START {'>'} NOW | +{MIN_OFFSET_MINUTES}MIN
            </div>
          </div>
          <div className="mt-6 grid gap-4 sm:grid-cols-3">
            {stats.map((stat) => (
              <div key={stat.label} className={clsx('rounded-2xl border bg-black/30 p-4 text-center', stat.border)}>
                <p className="text-[10px] font-tech uppercase tracking-[0.4em] text-white/60">{stat.label}</p>
                <p className={clsx('mt-2 text-3xl font-mono', stat.accent)}>{stat.value}</p>
              </div>
            ))}
            <div className="rounded-2xl border border-white/10 bg-black/20 p-4 text-center">
              <p className="text-[10px] font-tech uppercase tracking-[0.4em] text-white/60">Total</p>
