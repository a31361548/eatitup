...
export default function MembersClient(): React.ReactElement {
  ...
  const fetchMembers = useCallback(
    async (pageValue = page, searchValue = search) => {
      setLoading(true)
      setError(null)
      try {
    ...
    <div className="space-y-6">
                  ...
                  <tr key={member.id} className="border-t border-white/10">
                    ...
                    <td className="px-4 py-4 text-right">
                      <div className="flex justify-end gap-2">
                        <TechButton variant="secondary" className="!px-3 !py-1 text-[11px]" onClick={() => openEdit(member)}>
                          編輯
                        </TechButton>
                        <TechButton variant="ghost" className="!px-3 !py-1 text-[11px]" onClick={() => openReset(member)}>
                          重設密碼
                        </TechButton>
                      </div>
                    </td>
      ...
      <div className="flex items-center justify-between text-sm text-white/70">
        ...
        <div className="flex items-center gap-2">
          <TechButton variant="ghost" className="!px-3 !py-1 text-[11px]" onClick={() => { setPage((p) => Math.max(1, p - 1)); fetchMembers(page - 1, search) }} disabled={page <= 1}>
            上一頁
          </TechButton>
          <span>
            {page} / {totalPages}
          </span>
          <TechButton variant="ghost" className="!px-3 !py-1 text-[11px]" onClick={() => { setPage((p) => Math.min(totalPages, p + 1)); fetchMembers(page + 1, search) }} disabled={page >= totalPages}>
            下一頁
          </TechButton>
        </div>
      </div>
      ...
      {modal && (
        <Modal title={modal.type === 'create' ? '新增成員' : modal.type === 'edit' ? '編輯成員' : '重設密碼'} onClose={closeModal}>
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <TechButton variant="primary" onClick={submitMember} disabled={modalLoading}>
              {modalLoading ? '處理中…' : '儲存'}
            </TechButton>
          </div>
        </Modal>
...
function Modal({ title, onClose, children }: ModalProps): React.ReactElement {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 px-4">
      <div className="w-full max-w-lg rounded-3xl border border-white/10 bg-black/90 p-6 text-white shadow-[0_20px_60px_rgba(0,0,0,0.5)]">
